...
if (round=="evaluation"){
  ...
  train_set <- fread("C:/R/Rwork/sw/sw/sales_train_evaluation.csv")
  test_set <- fread("C:/R/Rwork/sw/sw/sales_test_evaluation.csv")
  weights <- fread("C:/R/Rwork/sw/sw/weights_evaluation.csv")
}else{
...
base_forecasts <- fread("C:/R/Rwork/sw/sw/3y_TX_3(T).csv")
weights$RMSSE <- NA
for (tsid in 1:nrow(weights)){
  ...
  if (weights$Level_id[tsid]=="Level12"){
    ...
    frc <- setNames(base_forecasts[tsid],names(cbind(tmp[,1:5], t(frc)))) #Store base forecasts
    frc <- as.numeric(frc[,6:ncol(frc)])
...
print(weights[weights$Level_id=="Level1",]$WRMSSE)
print(sum(weights[weights$Level_id=="Level2",]$WRMSSE))
print(sum(weights[weights$Level_id=="Level3",]$WRMSSE))
print(sum(weights[weights$Level_id=="Level4",]$WRMSSE))
print(sum(weights[weights$Level_id=="Level5",]$WRMSSE))
print(sum(weights[weights$Level_id=="Level6",]$WRMSSE))
print(sum(weights[weights$Level_id=="Level7",]$WRMSSE))
print(sum(weights[weights$Level_id=="Level8",]$WRMSSE))
print(sum(weights[weights$Level_id=="Level9",]$WRMSSE))
print(sum(weights[weights$Level_id=="Level10",]$WRMSSE))
print(sum(weights[weights$Level_id=="Level11",]$WRMSSE))
print(sum(weights[weights$Level_id=="Level12",]$WRMSSE))
